...
function MathText({ text }: { text: string }) {
  ...
  return (
    <>
      ...
      <span className="katex-content leading-relaxed" dangerouslySetInnerHTML={{ __html: html }} />
    </>
  )
}
...
export default function IntakeTestClient({ questions }: { questions: any[] }) {
  ...
  async function handleSubmit() {
    if (answered < total && !confirmed) {
      setError(`Bạn còn ${total - answered} câu chưa trả lời. Nhấn "Nộp bài ngay" để xác nhận.`)
      setConfirmed(true)
      return
    }
    setError(''); setConfirmed(false); setSubmit(true)
    const res = await submitIntakeTest(answers)
    if (res.error) { setError(res.error); setSubmit(false); return }
    router.replace('/student/intake/result')
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 p-6 max-w-2xl mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4 text-gray-900 text-base">
        ...
        <div className="space-y-3">
          {(['A','B','C','D'] as const).map(opt => {
            ...
            return (
              <button key={opt}
                onClick={() => { setAnswers(a => ({ ...a, [q.id]: opt })); setConfirmed(false); setError('') }}
                ...
                <span className="text-sm text-gray-800 mt-0.5"><MathText text={text} /></span>
              </button>
            )
          })}
        </div>
          ...
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            {confirmed && (
              <button onClick={handleSubmit} disabled={submitting}
                className="mt-2 text-sm text-amber-800 font-semibold underline">
                Nộp bài ngay →
              </button>
            )}
          </div>
