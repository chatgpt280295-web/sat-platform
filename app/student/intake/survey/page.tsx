...
export default function SurveyPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await saveSurvey(new FormData(e.currentTarget))
    setLoading(false)
    if (res.error) { setError(res.error); return }
    router.push('/student/intake/test')
  }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="school" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                placeholder="Tên trường của bạn" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Khối lớp</label>
              <select name="grade" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select name="english_level" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
            ...
            <div>
              ...
              <select name="sat_target" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
          ...
          <div>
            ...
            <select name="hours_per_week" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
